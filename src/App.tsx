...
const experience = [
  {
    ...
    company: 'UnstableML',
    copy: 'Building practical AI-assisted software and agent workflows, with an eye for reliable systems and useful human experiences.',
    tag: 'Now',
  },
  {
    period: '2025 — Present',
    role: 'Technical Co-founder',
    company: 'First Step Healthcare Academy',
    copy: 'Designed and shipped an enrollment platform that brings program discovery, registration, payments, and operations into one clear flow.',
    tag: 'Venture',
  },
  {
    ...
    copy: 'Developing a foundation in computing while following questions that sit between science, software, and AI.',
    tag: 'Education',
  },
  {
    period: 'Oct 2025',
    role: 'Selected Participant',
    company: 'HackHarvard · Cambridge, MA',
    copy: 'A high-energy build weekend dedicated to learning fast, making, and meeting people who care about the craft.',
    tag: 'Community',
  },
];
...
const projects = [
  ...
  },
  {
    ...
    detail: 'Reads tracebacks and AST structure to turn an opaque failure into a useful next move.',
...
const skillGroups = [
  ['Build', 'Python · TypeScript · React · Node.js · SQL · C++20'],
  ['Explore', 'SciPy · scikit-learn · data pipelines · simulation · AI tooling'],
    ...
    <main>
      ...
      <section className="about section" id="about">
        <SectionIntro overline="A little about me" title="I like following a good question.">
          My best work starts where an interesting problem meets someone who needs a clearer tool. I enjoy turning messy systems into calm, useful experiences.
        </SectionIntro>
        <Reveal className="about-grid" delay={0.08}>
          <div className="about-statement"><span>01</span><p>Research-minded, product-aware, and always ready to learn the shape of a new problem.</p></div>
          <div className="about-details"><p>Right now I’m studying computer science at CSUF and building across AI, scientific computing, and the web.</p><p>I’m especially interested in tools that make hard work more legible: better debugging, more reproducible research, and simple paths through complex workflows.</p><ExternalLink href={links.linkedin} className="text-link">More on LinkedIn <ArrowUpRight /></ExternalLink></div>
        </Reveal>
      </section>

      <section className="experience section" id="experience">
        <SectionIntro overline="Building in the open" title="Experience">
          Each role has made the work feel more human: a lesson in craft, collaboration, or the responsibility behind what we ship.
        </SectionIntro>
        <div className="experience-list">
          {experience.map((item, index) => <Reveal key={item.role} delay={index * 0.05}><article className="experience-item"><div className="experience-index">{String(index + 1).padStart(2, '0')}</div><div><p className="period">{item.period} <span>{item.tag}</span></p><h3>{item.role}</h3><h4>{item.company}</h4></div><p className="experience-copy">{item.copy}</p></article></Reveal>)}
        </div>
      </section>

      <section className="work section" id="work">
        <SectionIntro overline="Things I’ve made" title="Selected work">
          Projects where an idea needed a real home — built through research, small experiments, and a lot of iteration.
        </SectionIntro>
        <div className="project-grid">
          {projects.map((project, index) => <Reveal key={project.name} delay={(index % 2) * 0.08}><article className="project-card"><div className="project-image"><img src={project.image} alt="" /><span>{project.number}</span></div><div className="project-body"><p className="project-category">{project.category}</p><h3>{project.name}</h3><p className="project-summary">{project.description}</p><p className="project-detail">{project.detail}</p><div className="project-bottom"><div className="tags">{project.stack.map((tag) => <span key={tag}>{tag}</span>)}</div><ExternalLink href={project.href} className="project-open" aria-label={`Open ${project.name}`}><ArrowUpRight /></ExternalLink></div></div></article></Reveal>)}
        </div>
      </section>
      ...
      <section className="skills section" id="skills">
        ...
        <SectionIntro overline="My toolkit" title="Technical, but never just technical.">
          The languages and tools are a means to an end: useful ideas, careful details, and software people can keep coming back to.
        </SectionIntro>
